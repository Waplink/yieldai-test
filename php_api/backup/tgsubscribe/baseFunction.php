...
class baseFunction
{
	...
	function __construct() {
		...
		$this->YieldaiApiUrl = 'https://yieldai-test.vercel.app/api/public/v1/wallet/';
	 ...
	 */
	public function formatWalletBalance($data=[]) 
	{
		...
		foreach ($data['balance']['tokens'] as $token) {
			...
			$amount = $token['amount'];
			$price = number_format((float)$token['priceUSD'], 4);
			$value = number_format((float)$token['valueUSD'], 4);
			...
			$totalUsd += (float)$token['valueUSD'];
		}
		
		$totalFormatted = number_format($totalUsd, 4);
		
		$message .= "\n\n";
		$message .= "💰 *TOTAL VALUE: \${$totalFormatted}*\n";
		$message .= "\n\n";
		
		$message .= $this->formatInvestmentPortfolio($data['protocols'], $totalUsd);
		...
		switch ($protocol) {
			...
			case 'aries':
				if (isset($position['coin'])) {
					...
					// Для echelon/aries нет прямой USD стоимости, используем количество
					$result['valueUSD'] += (float)$amount;
				}
				break;
				...
				if (isset($position['fees']['unclaimed']) && is_array($position['fees']['unclaimed'])) {
					foreach ($position['fees']['unclaimed'] as $fee) {
						if (isset($fee['amountUSD']) && (float)$fee['amountUSD'] > 0) {
							...
							$result['message'] .= "  ┃ ┣ {$feeEmoji} Unclaimed Fees: $" . number_format($feeUsd, 4) . "\n";
				...
				if (isset($position['farm']['unclaimed']) && is_array($position['farm']['unclaimed'])) {
					foreach ($position['farm']['unclaimed'] as $reward) {
						if (isset($reward['amountUSD']) && (float)$reward['amountUSD'] > 0) {
							...
							$result['message'] .= "  ┃ ┗ 🎁 Farm Reward: $" . number_format($rewardUsd, 4) . "\n";
					...
					if (isset($position['estimatedCollectFees']) && is_array($position['estimatedCollectFees'])) {
						foreach ($position['estimatedCollectFees'] as $fee) {
							if (isset($fee['symbol']) && isset($fee['usd']) && $fee['usd'] > 0) {
								...
								$result['message'] .= "  ┃ ┣ {$emoji} Pending Fees: $" . number_format($feeUsd, 4) . "\n";
								$result['valueUSD'] += $feeUsd;
							}
						}
					}
					...
					if (isset($position['estimatedIncentives']) && is_array($position['estimatedIncentives'])) {
						foreach ($position['estimatedIncentives'] as $incentive) {
							if (isset($incentive['symbol']) && isset($incentive['usd']) && $incentive['usd'] > 0) {
								...
								$result['message'] .= "  ┃ ┣ 🎯 Incentive: {$emoji} $" . number_format($incentiveUsd, 4) . "\n";
	 ...
	 */
	public function formatInvestmentPortfolio($data=[]) 
	{
		if (empty($data) || !is_array($data) || empty($data['protocols'])) {
			return false;
		}

		$message = "📈 *INVESTMENT PORTFOLIO*\n";
		$message .= "\n";
		$message .= "📊 Total Protocols: {$data['protocolsTotal']}\n";
		$message .= "✅ Active: {$data['protocolsWithPositions']}\n";
		if (isset($data['failedProtocols']) && $data['failedProtocols'] > 0) {
			$message .= "❌ Failed: {$data['failedProtocols']}\n";
		}
		$message .= "\n";
		
		$hasAnyPositions = false;
		$totalAssets = 0;
		$totalRewards = 0; // Отдельно подсчитываем rewards
		...
		foreach ($data['protocols'] as $protocol) {
			if ($protocol['success'] && $protocol['positionsCount'] > 0) {
				...
				foreach ($protocol['positions'] as $position) {
					...
					$totalAssets += (float)$array['valueUSD'];
				}
			}
		}
		...
		$totalFormatted = number_format($totalAssets, 2);
		$message .= "\n";
		$message .= "\n";
		$message .= "💰 *TOTAL ASSETS: \${$totalFormatted}*\n";
		$message .= "\n";
