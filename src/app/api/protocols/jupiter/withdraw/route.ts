...
type JupiterWithdrawInstructionResponse = {
  instructions?: Array<{
    programId: string;
    accounts: Array<{
      pubkey: string;
...
async function buildLegacyTransactionFromInstruction(input: {
  ...
  const upstream = await fetch("https://lite-api.jup.ag/lend/v1/earn/withdraw-instructions", {
    method: "POST",
    headers: {
  ...
  const payload = (await upstream.json().catch(() => null)) as JupiterWithdrawInstructionResponse | null;
  ...
  for (const ix of instructions) {
    ...
    // Since withdrawer already has USDG balance, skip ATA instructions entirely.
